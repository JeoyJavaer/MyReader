...
abstract class CommonRecyclerAdapter<ITEM>(protected val context: Context) :
    ...
    fun addFooterView(footerView: View) {
        synchronized(lock) {
            ...
            footerItems?.let {
                ...
                it.put(TYPE_FOOTER_VIEW + it.size(), footerView);
                notifyItemInserted(index);
            }
        }
    }
    ...
    final override fun getItemViewType(position: Int): Int {
        return when {
            ...
            isFooter(position) -> TYPE_FOOTER_VIEW + position - getActualItemCount() - getFooterCount()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
        return when {
            ...
            viewType >= TYPE_FOOTER_VIEW -> {
                ...
                itemDelegates.getValue(viewType).registerListener(holder)
            ...
            manager.spanSizeLookup =object :GridLayoutManager.SpanSizeLookup(){
                override fun getSpanSize(position: Int): Int {
                    return   getItem(position)?.let {
                        if (isHeader(position)|| isFooter(position)) manager.spanCount else getSpanSize(it,getItemViewType(position),position)
                    }?:manager.spanCount
                }
    ...
    protected open  fun  startAnimation(holder: ItemViewHolder,item: ItemAnimation){
        item.itemAnimation?.let {
            for (anim in it.getAnimators(holder.itemView)){
                anim.setDuration(item.itemAnimDuration)
